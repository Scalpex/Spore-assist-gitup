...
function updatePlayerButtons() {
    ...
    players.forEach((player, index) => {
     ...
     if (selectedPlayerIndex !== null) {
         ...
         nextButton.textContent = `${selectedPlayer.name} est le hacker`;
     } else {
...
document.getElementById('nextButton').addEventListener('click', () => {
    if (selectedPlayerIndex !== null) {
        players[selectedPlayerIndex].role = "hack"; // Attribuer le rôle de mutant
        savePlayersToLocalStorage();
        window.location.href = 'hack.html'; // Rediriger vers la page para
    }
    else {window.location.href = 'at_fana.html';}
});
