...
function updatestatusMessage() {
    ...
    if (hack.status === 'para') {
        ...
        statusMessageDiv.innerHTML = 'Choisissez une cible à hacker (pas 2x de suite la même)';
...
// Fonction pour afficher la phrase en fonction de l'action sélectionnée
function updatresultMessage() {
    ...
    if (actionSelected === 'info') {
        ...
        resultMessage.innerHTML = `(Le nombre de mutant(s) à bord est de : ${mutantsCount}.)`;
        }
    } else if (actionSelected === 'psy') {
        const psy = players.find(player => player.role === 'psy');
        if (psy.status === 'mort' || psy.status === 'mut' || psy.status === 'para') {
            resultMessage.innerHTML = `(Le psychologue est innactif cette nuit.)`;
        } else {
            const inspectionResult = psykedResult === 'mut' ? 'mutant' : (psykedResult === 'mort' ? 'mort' : 'sain');
            resultMessage.innerHTML = `(le Psy a inspecté ${psykedName} et le résultat est : ${inspectionResult}.)`;
        }
        ...
        if (gene.status === 'mort' || gene.status === 'mut' || gene.status === 'para') {
            resultMessage.innerHTML = `(Le généticien est innactif cette nuit.)`;
        } else {
            const genomeResult = genedResult === 'hôte' ? 'hôte' : (genedResult === 'résist' ? 'résistant' : 'neutre');
            resultMessage.innerHTML = `(le Généticien a inspecté ${genedName} et le résultat est : ${genomeResult}.)`;
        }
    }
}
