...
function updatePlayerButtons() {
    ...
    players.forEach((player, index) => {
     ...
     if (selectedPlayerIndex !== null) {
         ...
         nextButton.textContent = `${selectedPlayer.name} est l'indic`;
     } else {
...
document.getElementById('nextButton').addEventListener('click', () => {
    if (selectedPlayerIndex !== null) {
        players[selectedPlayerIndex].role = "indic"; // Attribuer le rôle de mutant
        savePlayersToLocalStorage();
        window.location.href = 'indic.html'; // Rediriger vers la page para
    }
    else {window.location.href = 'at_journ.html';}
});
